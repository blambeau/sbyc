module Formulae
  class And < Formulae::Term
    
    attr_reader :terms
    
    # Are terms normalized?
    def self.normalized?(terms)
      names = terms.select{|t| Formulae::NamedTerm === t}.collect{|n| n.name}
      names == names.uniq
    end
    
    def self.new(*terms)
      terms = terms.flatten.uniq.delete_if{|v| v == Formulae::ALL}
      if terms.empty?
        Formulae::ALL
      elsif terms.size == 1
        terms.first
      elsif terms.include?(Formulae::NONE)
        Formulae::NONE
        ...
        named, others = terms.partition{|term| Formulae::NamedTerm === term}
        hash = Hash.new{|h,k| h[k] = Formulae::ALL}
